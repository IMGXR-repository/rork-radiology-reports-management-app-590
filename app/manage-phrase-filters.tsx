...
import { PhraseCategory, PhraseFilter } from '@/types';
import { lightTheme, darkTheme } from '@/constants/theme';

export default function ManagePhraseFiltersScreen() {
  const { phraseCategories, phraseFilters, savePhraseCategories, savePhraseFilters, settings } = useApp();
  ...
  const handleToggleCategoryVisibility = async (categoryId: string) => {
    const updatedCategories = phraseCategories.map(cat =>
      cat.id === categoryId ? { ...cat, isVisible: !cat.isVisible } : cat
    );
    await savePhraseCategories(updatedCategories);
  };

  const handleToggleFilterActive = async (filterId: string) => {
    const updatedFilters = phraseFilters.map(filter =>
      filter.id === filterId ? { ...filter, isActive: !filter.isActive } : filter
    );
    await savePhraseFilters(updatedFilters);
  };
  ...
  const handleAddCategory = async () => {
    ...
    await savePhraseCategories([...phraseCategories, newCategory]);
    setNewCategoryName('');
  ...
  const handleAddFilter = async () => {
    ...
    const newFilter: PhraseFilter = {
      id: `phrase_filter_${Date.now()}`,
    ...
    await savePhraseFilters([...phraseFilters, newFilter]);
    setNewFilterName('');
  ...
  const handleDeleteCategory = (categoryId: string) => {
    const categoryFilters = phraseFilters.filter(f => f.categoryId === categoryId);
  ...
  const deleteCategory = async (categoryId: string) => {
    const updatedCategories = phraseCategories.filter(cat => cat.id !== categoryId);
    const updatedFilters = phraseFilters.filter(filter => filter.categoryId !== categoryId);
    await savePhraseCategories(updatedCategories);
    await savePhraseFilters(updatedFilters);
  };
  ...
  const deleteFilter = async (filterId: string) => {
    const updatedFilters = phraseFilters.filter(filter => filter.id !== filterId);
    await savePhraseFilters(updatedFilters);
  };

  const getFiltersForCategory = (categoryId: string) => {
    return phraseFilters.filter(filter => filter.categoryId === categoryId);
  };
  ...
  return (
    <View style={[styles.safeContainer, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.container, { backgroundColor: theme.background }]}>
        <Stack.Screen
          options={{
            title: 'Gestionar Filtros de Frases',
